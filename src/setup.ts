...
/**
 * Render the OpenCode plugin that gives live recording parity with Claude Code's hooks.
 *
 * OpenCode has no hook-command mechanism like Claude Code, but its plugin API exposes the two
 * injection points we need. The plugin shells out to the SAME pinned `clancey hook` CLI the
 * Claude Code hooks use, translating OpenCode's tool calls into the Claude hook payload — so the
 * whole recording/nudge contract (runHook) is reused unchanged:
 *
 *   - experimental.chat.system.transform → push the SessionStart "record decisions/learnings"
 *     instruction into the system prompt (fetched once, memoized for the process).
 *   - tool.execute.after → record the tool event and append any decision nudge to the tool
 *     output so the model sees it. Filtered to edit/write/bash to mirror Claude's PostToolUse
 *     matcher (the hook fires after every tool otherwise, including read/grep/glob).
 *
 * Pinned to `spec` so a user's plugin keeps working on the version that set it up, exactly like
 * the Claude Code hook command. Regenerated verbatim on every setup, so re-running re-pins it.
 */
export function renderOpencodePlugin(spec: string): string {
  return `// Generated by clancey setup — do not edit. Re-run \`npx ${spec} setup\` to update.
// Gives OpenCode live decision/learning recording parity with Claude Code by bridging to the
// pinned \`clancey hook\` CLI (same contract Claude Code's hooks use).
import { spawn } from "node:child_process";

const SPEC = ${JSON.stringify(spec)};

/** Invoke \`clancey hook\` with a Claude-shaped payload on stdin; resolve its parsed stdout JSON. */
function runHook(payload) {
  return new Promise((resolve) => {
    let child;
    try {
      child = spawn("npx", ["-y", SPEC, "hook"], { stdio: ["pipe", "pipe", "ignore"] });
    } catch {
      resolve(null);
      return;
    }
...
export const ClanceyPlugin = async ({ directory, worktree }) => {
  ...
  let instruction; // memoized once per process

  return {
    "experimental.chat.system.transform": async (input, output) => {
      try {
        if (instruction === undefined) {
          const res = await runHook({ hook_event_name: "SessionStart", session_id: input.sessionID || "", cwd });
          instruction = (res && res.hookSpecificOutput && res.hookSpecificOutput.additionalContext) || null;
        }
    ...
    "tool.execute.after": async (input, output) => {
      try {
        ...
        const nudge = res && res.hookSpecificOutput && res.hookSpecificOutput.additionalContext;
...
export function configureOpencodePlugin(spec: string): { result: "added" | "updated"; file: string } {
  const file = opencodePluginPath();
  const had = fs.existsSync(file);
  fs.mkdirSync(path.dirname(file), { recursive: true });
  fs.writeFileSync(file, renderOpencodePlugin(spec));
...
async function selectTargets(preset: Target[] | null): Promise<Target[]> {
  ...
  const selected = await multiselect<"all" | Target>({
    ...
    options: [
      ...
      { value: "opencode", label: "OpenCode", hint: detected.opencode ? "MCP + live recording" : "not detected" },
    ],
...
export async function setup(opts: { cleanLegacy?: boolean; targets?: Target[] } = {}): Promise<void> {
  ...
  if (targets.includes("opencode")) {
    ...
    const plugin = configureOpencodePlugin(spec);
    clog.success(
      plugin.result === "added"
