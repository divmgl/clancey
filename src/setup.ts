 ...
 * tool.execute.after. The plugin shells out to the SAME pinned `clancey hook` CLI Claude
 * Code uses, translating OpenCode tool calls into the Claude-shaped hook payload — so
 * live tool recording is shared. Decision/learning recording is left to MCP tool contracts
 * (no system-prompt or tool-output coaching).
 *
 * Pinned to `spec` so a user's plugin keeps working on the version that set it up.
 * Regenerated verbatim on every setup, so re-running re-pins it.
 */
...
// Records OpenCode file/command tool events via the pinned \`clancey hook\` CLI.
...
/** Render the Grok Build MCP server block (stdio via pinned npx). */
export function renderGrokMcpBlock(spec: string): string {
  return [
    "[mcp_servers.clancey]",
    `command = ${tomlString("npx")}`,
    `args = [${["-y", spec].map(tomlString).join(", ")}]`,
    "enabled = true",
    "",
  ].join("\n");
}
...
export function configureGrok(spec: string): "added" | "updated" {
  ...
  const block = renderGrokMcpBlock(spec);
...
/**
 * Write (or re-pin) the Grok hooks file for live tool-event recording.
 * PostToolUse only — decision coaching is tool-description driven, not hook-injected.
 */
export function configureGrokHooks(spec: string): { result: "added" | "updated"; file: string } {
...
async function selectTargets(preset: Target[] | null): Promise<Target[]> {
  ...
  const selected = await multiselect<"all" | Target>({
    ...
    options: [
      ...
      { value: "codex", label: "Codex", hint: detected.codex ? "MCP + history import" : "not detected" },
      { value: "opencode", label: "OpenCode", hint: detected.opencode ? "MCP + live recording" : "not detected" },
      { value: "grok", label: "Grok Build", hint: detected.grok ? "MCP + live recording" : "not detected" },
    ],
...
export async function setup(opts: { cleanLegacy?: boolean; targets?: Target[] } = {}): Promise<void> {
  ...
  if (targets.includes("grok")) {
    const grok = configureGrok(spec);
    clog.success(
      grok === "added"
        ? `Registered Grok Build MCP server (${spec}) in ${tildify(GROK_CONFIG())}`
        : `Re-pinned Grok Build MCP server to ${spec} in ${tildify(GROK_CONFIG())}`,
    );
    const hooks = configureGrokHooks(spec);
    clog.success(
      hooks.result === "added"
